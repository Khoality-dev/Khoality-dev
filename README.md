<div align="center">

<img src="./neural-profile.svg?v=5" alt="Anh Khoa Nguyen - Software Engineer and AI Engineer specializing in high-performance C++, CUDA, and AI/ML systems" width="880" />

### Software Engineer · AI Engineer

</div>

### Hi, I'm Khoa 😄

I'm a software engineer and AI engineer who loves developing new ideas and turning them into useful technology. Always learning, always building - a proud learnaholic.

**More about my work, experience, and ideas → [Explore my portfolio](https://portfolio.kurikuri.sbs/)**

Connect with me on [LinkedIn](https://www.linkedin.com/in/anhkhoan).

### Tech

`C++` · `Python` · `TypeScript` · `C#` · `CUDA` · `PyTorch` · `TensorRT` · `ONNX Runtime`

`Computer Vision` · `Generative AI` · `Diffusion Models` · `LLMs` · `AI Agents` · `Docker` · `MCP`
